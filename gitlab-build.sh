	...
	if [ "x$1" == "xpublish" ]; then
		...
		if [ "$CI_COMMIT_TAG" != "" ]; then
			...
			if [ "$VER_DIR" = "latest" ]; then
				rsync -e 'ssh -p 45' -vP public/index.html docs@beagleboard.org:/var/www/docs/
			fi
			rsync -e 'ssh -p 45' -avP --delete public/$VER_DIR/. docs@beagleboard.org:/var/www/docs/$VER_DIR
		fi
	fi
}
